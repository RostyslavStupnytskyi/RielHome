...
function makeRequest() {
    $.ajax({
        url: `${API_URL}/region?page=${currentPage}&size=20`,
        type: 'get',
        success: function(response) {
            $pageBtnContainer.html('');
            $tableBody.html('');
            totalPages = response.totalPages;
            appendRegionsToTable(response.data);
            actionsOnUpdateBtn();
...
function appendRegionToTable(region) {
    $tableBody.append(` 
            <tr class="table-component">
                <th class="id">${region.id}</th>
                <th class="name"> <input type="text" class="table-input" value="${region.name}" disabled></th>
                <th class="actions"> <button value="${region.id}" class="update-btn btn">Update</button> <button value="${region.id}" class="delete-btn btn">Delete</button></th>
            </tr>
    `)
}
...
function appendRegionsToTable(regions){
